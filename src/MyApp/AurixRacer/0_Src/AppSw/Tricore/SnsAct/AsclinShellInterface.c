...
boolean AppShell_pidout(pchar args, void *data, IfxStdIf_DPipe *io);
...
const Ifx_Shell_Command AppShell_commands[] = {
    ...
    {"ls1", "      : LineScan1", &g_AsclinShellInterface,       &AppShell_linescan1,    },
    {"mls", "      : Monitoring LineScan", &g_AsclinShellInterface,       &AppShell_monlinescan,    },
	{"pid", "      : Monitoring Pidout", &g_AsclinShellInterface,       &AppShell_pidout,    },
    {"vadc", "     : Vadc Backgound 1", &g_AsclinShellInterface,       &AppShell_vadcbg1,    },
    {"enc", "      : Encoder", &g_AsclinShellInterface,       &AppShell_enc,    },
    {"p00_0", "    : Port00_0", &g_AsclinShellInterface,       &AppShell_port00_0,    },
    {"p00_1", "    : Port00_1", &g_AsclinShellInterface,       &AppShell_port00_1,    },
...
boolean AppShell_pidout(pchar args, void *data, IfxStdIf_DPipe *io)
{
	...
	if (Ifx_Shell_matchToken(&args, "?") != FALSE)
    {
        IfxStdIf_DPipe_print(io, "  Syntax     : pid period_ms"ENDL);
    }
    else
    {
    	if(Ifx_Shell_parseSInt32(&args, &period_ms) != FALSE){
    		g_motor.period_ms = period_ms;
    		g_motor.enable = TRUE;
    	}else
    	{
    		g_LineScan.enable = FALSE;
    	}
    	IfxStdIf_DPipe_print(io, "  pid: %4d "ENDL, g_motor.period_ms);
    }
...
void AsclinShellInterface_runPidScan(void)
{
	sint32 i;
	static sint32 cnt;

	if(g_motor.enable == TRUE)
	{
		cnt--;
		if(cnt < 0){
			cnt = (sint32) g_motor.period_ms/10;

//			IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%5d",(int)g_motor.f32pid_out);
		//	IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%5d",(int)g_motor.f32cur_vel_avr);
		//	IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, ",%5d",(int)g_motor.f32user_vel);
		//	IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, ",%3d",(int)(g_motor.f32kp*100));
		//	IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, ",%3d"ENDL,(int)(g_motor.f32kd*100));

					//IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%3d"ENDL,(int)(g_lcsen[i].int32result));
					//IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%3d"ENDL,(int)(g_lcsen[i].int32result));
			for (i = 0; i < 100; i++) IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%3d"ENDL,(int)(g_lcsen[i].int32result));
					//IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%3d"ENDL,(int)(g_rcsen[i].int32diff));
				//IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "%3d",(int)g_int32dis_result[0]);
				//IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, ",%3d"ENDL,(int)g_int32dis_result[2]);
					//IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, ",%3d"ENDL,(int)(g_rcsen[i].u32active) * 4095);

//			VFDPrintf("%5d", g_motor.f32pid_out);
		}
	}
}
